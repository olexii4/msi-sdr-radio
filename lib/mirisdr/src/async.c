...
failed:
    if (p && p->async_status == MIRISDR_ASYNC_RUNNING) {
...
retry_transfer:
    ...
    switch (p->transfer) {
    case MIRISDR_TRANSFER_BULK:
        fprintf( stderr, "libmirisdr: using BULK transfer mode\n");
        ...
        break;
    case MIRISDR_TRANSFER_ISOC:
        fprintf( stderr, "libmirisdr: using ISOC transfer mode\n");
    ...
    for (i = 0; i < p->xfer_buf_num; i++) {
        ...
        if (r < 0) {
            ...
            if (p->transfer == MIRISDR_TRANSFER_ISOC) {
...
failed_free:
    ...
    if (p->xfer) {
        ...
        {
            struct timeval drain_tv = {2, 0};
            int drain_tries = 0;
            while (drain_tries < 50) {
                int done = 1;
                for (j = 0; j < p->xfer_buf_num; j++) {
